...
pair<int, vector<int> > dijkstra(map<int, vector<pair<int, int> > >& graf, int asal, int tujuan) {
    ...
    if (jarak[tujuan] != numeric_limits<int>::max()) {
        for (int at = tujuan; at != asal; at = pendahulu[at]) {
            rute.push_back(at);
        }
        rute.push_back(asal);
...
int main() {
    ...
    pair<int, vector<int> > hasil = dijkstra(graf, simpulAsal, simpulTujuan);
    int jarakTerpendek = hasil.first;
    vector<int> ruteTerpendek = hasil.second;

    if (jarakTerpendek != numeric_limits<int>::max()) {
        cout << "Jarak dari simpul " << simpulAsal << " ke simpul " << simpulTujuan << " adalah " << jarakTerpendek << endl;
        cout << "Rute terpendek: ";
        for (size_t i = 0; i < ruteTerpendek.size(); ++i) {
            if (i > 0) cout << " -> ";
            cout << ruteTerpendek[i];
        }
        cout << endl;
    } else {
